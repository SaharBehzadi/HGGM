...
I_B=[round(Features*0.7)+1:Features];
I_n=[];
I_g=[];
dependency=5;
...
for length=10:1:10
    ...
    for itr=1:50
        ...
        Filename = strcat( 'Length_',num2str(traning_data_size),'_Poisson',num2str(size(I_p,2)),'_Bernoulli',num2str(size(I_B,2)),'_10.mat');
        ...
        disp(strcat('TS_length = ',num2str(traning_data_size)));
...
%                     F_measure_AD_Thrsh_mean=Fmeasure(adj_Thrsh_mean_AD,Ground_Truth);
                    ...
                    ComparingTable(count,:)= [strength traning_data_size L F_measure_AD F_measure_Arnold F_measure_Kim ...
